export default function Validator() {
  ...
  if (nameAction.value.length > 25 || !nameAction.value) {
    alert('Введите название мероприятия! Максимальное колличество символов - не более 25');
  ...
  } else if ((start.value.replace(/:\d{2}/, "") - end.value.replace(/:\d{2}/, "") > 0)) { //часы
    alert('Мероприятие должно закончиться после времени начала!(часы)');
  } else if ((start.value.replace(/:\d{2}/, "") - end.value.replace(/:\d{2}/, "") === 0) && (start.value.replace(/\d{2}:/, "") - end.value.replace(/\d{2}:/, "") > 0)) { //минуты
